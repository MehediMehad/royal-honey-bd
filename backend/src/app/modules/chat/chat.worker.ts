...
export const setupChatWorker = () => {
  const worker = new Worker(
    ...
    async (job: Job<IProcessMessageJob>) => {
      ...
      // 0. Per-customer active worker mutex lock to prevent concurrent parallel replies
      const activeWorkerLockKey = `lock:chat:worker:${channel}:${channelId}`;
      const lockAcquired = await redis.set(activeWorkerLockKey, job.id || 'ACTIVE', 'EX', 20, 'NX');
      if (!lockAcquired) {
        console.log(`⏸️ [ChatWorker] Customer ${channel}:${channelId} is already being processed by another worker. Skipping duplicate concurrent run.`);
        return { status: 'skipped', reason: 'concurrent_worker_active' };
      }

      try {
        // Drain any rapid buffered messages sent by this customer
        const bufferKey = `chat:msg_buffer:${channel}:${channelId}`;
        let bufferedTexts: string[] = [];
        try {
          const rawBuffer = await redis.lrange(bufferKey, 0, -1);
          await redis.del(bufferKey);
          if (rawBuffer && rawBuffer.length > 0) {
            bufferedTexts = rawBuffer
              .map((raw) => {
                try {
          ...
          console.warn('⚠️ [ChatWorker] Redis buffer drain error:', bufErr);
        }

        // Combine buffered texts, removing consecutive duplicate words (e.g. ["Hlw", "Hlw"] -> "Hlw")
        let initialText = content || '';
        if (bufferedTexts.length > 0) {
          const uniqueTexts: string[] = [];
          for (const t of bufferedTexts) {
            const trimmed = t.trim();
            if (trimmed && !uniqueTexts.includes(trimmed)) {
              uniqueTexts.push(trimmed);
            }
          }
          if (uniqueTexts.length > 0) {
            initialText = uniqueTexts.join('\n');
          }
        }

        // 1. Identify/Create Customer and Conversation
        const { customer, conversation } =
          await ChatServices.getOrCreateCustomerAndConversation({
            ...job.data,
            content: initialText,
          });
        ...
        if (hasAllDetails && extracted.isOrderConfirmed && summaryWasShown) {
          ...
          if (
            ...
            try {
              ...
              aiReply = `দুঃখিত স্যার! এই মুহূর্তে অর্ডার সম্পন্ন করতে একটু সমস্যা হয়েছে। অনুগ্রহ করে আমাদের হেল্পলাইনে (01604121107) একটু জানান, আমরা ঠিক করে দিচ্ছি।`;
            }
          }
        }
        ...
        // Check cooldown: prevent sending another AI reply to the same customer within 3 seconds
        const replyCooldownKey = `chat:last_ai_reply:${channel}:${channelId}`;
        const lastReplyTime = await redis.get(replyCooldownKey);
        if (lastReplyTime && Date.now() - Number(lastReplyTime) < 3000) {
          console.warn(`🛡️ [ChatWorker] Throttled duplicate AI reply to ${channel}:${channelId} within 3s window.`);
          return { status: 'skipped', reason: 'reply_throttled' };
        }
        ...
        await redis.set(replyCooldownKey, Date.now().toString(), 'EX', 10);
        ...
        await redis.del(activeWorkerLockKey);
      }
    },
    {
